...
- [x] Configure `LANGUAGES = [("en", "English"), ("nl", "Nederlands"), ("fr", "Français")]`
      in `base.py`
...
- [x] Generate translation files:
  ```
  uv run python manage.py makemessages -l nl
  uv run python manage.py makemessages -l fr
  ```
- [x] Translate all existing strings to Dutch (`nl`) and French (`fr`) — ~100 msgids each
...
- [x] `uv run python manage.py compilemessages` — `locale/nl/django.mo` + `locale/fr/django.mo`
...
#### Post-Phase 4 locale fix
- [x] **Locale directories must match `LANGUAGES` codes exactly** — `makemessages -l nl_BE`
      creates `locale/nl_BE/` but `LANGUAGES` uses `nl`; Django resolves `Accept-Language: nl`
      to the `nl` locale directory, not `nl_BE`; directories renamed `nl_BE` → `nl` and
      `fr_BE` → `fr` so our translations override Django's built-in `nl`/`fr` catalogs
...
| 2026-02-22 | No `i18n_patterns` — header-based + session detection only                                    | `LocaleMiddleware` uses `Accept-Language` header and `django_language` session key (set by `set_language` view); no language prefix in URLs; works seamlessly without URL changes                                        |
